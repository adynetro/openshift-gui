  ...
  return (
    ...
    >
      <div className="bg-[#0f172a] border border-cyan-500/40 rounded-xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden">
        ...
        {/* Items List */}
        <div className="flex-1 overflow-auto p-3 space-y-1 divide-y divide-slate-800/40 font-sans">
          {items.length === 0 ? (
        ...
        <div className="p-3 bg-slate-900 border-t border-slate-800 flex items-center justify-between text-xs text-slate-400 font-mono">
          <div className="flex items-center gap-2">
            ...
            <span>•</span>
            <span><kbd className="px-1.5 py-0.5 rounded bg-slate-800 text-slate-200 border border-slate-700">↵ Enter</kbd> to select</span>
          </div>

          <button
            onClick={onClose}
            className="px-4 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium border border-slate-700 transition-colors"
          >
            Cancel
          </button>
        </div>
